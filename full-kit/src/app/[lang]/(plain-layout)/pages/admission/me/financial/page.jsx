          ...
          {transactions.length === 0 ? (
            ...
            <ul className="space-y-4">
              {transactions.map((tx) => {
                ...
                const isIncome = ["deposit", "refund", "hold_income"].includes(
                  tx.type
                )
                ...
                return (
                  <li
                    ...
                    className={`p-4 flex items-center justify-between rounded-xl shadow-sm border ${
                      isHold ? "bg-yellow-50 border-yellow-300" : "bg-gray-50"
                    } hover:bg-gray-100 transition`}
                  >
                    <Link
                      href={`/pages/admission/me/financial/transactions/${tx._id}`}
                      className="flex items-center gap-3 flex-1"
                    >
                      <div
                        className={`p-2 rounded-full shadow ${isHold ? "bg-yellow-100" : "bg-white"}`}
                      >
                        {isIncome ? (
                          <ArrowDownRight
                            className={`${
                              isHold ? "text-yellow-500" : "text-green-500"
                            } w-5 h-5`}
                          />
                        ) : (
                          <ArrowUpRight
                            className={`${
                              isHold ? "text-yellow-500" : "text-red-500"
                            } w-5 h-5`}
                          />
                        )}
                      </div>
