...
export default function useValidation(setFormData, setErrors) {
  ...
  const validateStep = (fields, formData) => {
    ...
    fields.forEach((field) => {
      ...
      if (field === "nationalId")
        stepErrors.nationalId = validateNationalId(formData.nationalId)
      if (field === "gender" && !formData.gender)
        stepErrors.gender = "الرجاء اختيار الجنس"
      if (field === "birthDate" && !formData.birthDate)
        stepErrors.birthDate = "الرجاء إدخال تاريخ الميلاد"
      if (field === "desiredGrade" && !formData.desiredGrade)
        stepErrors.desiredGrade = "الرجاء إدخال الصف المرغوب"
    })
    return stepErrors
  }
